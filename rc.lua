...
awful.screen.connect_for_each_screen(function(s)
	...
	local function is_laptop()
		...
		local file = io.open(home .. "/is_laptop", "r")
		if file then
			file:close()
			return true
		end
...
globalkeys = gears.table.join(
	...
	awful.key({}, "XF86AudioMute", function()
		awful.spawn.easy_async("pactl set-sink-mute @DEFAULT_SINK@ toggle", function()
			awful.spawn.easy_async_with_shell("pactl get-sink-mute @DEFAULT_SINK@", function(stdout)
				local mute = stdout:match("yes") and "Muted" or "Unmuted"
	...
	awful.key({}, "XF86MonBrightnessUp", function()
		...
		awful.spawn.easy_async_with_shell("brightnessctl get", function(stdout)
			...
			local brightness_percent = math.floor(brightness / 255 * 100)
			naughty.notify({ text = "Brightness: " .. brightness_percent .. "%" })
		end)
	end, { description = "increase brightness", group = "hotkeys" }),

	-- Decrease brightness with notification
	...
	awful.key({ altkey, "Control", "Shift" }, ".", function()
		os.execute("mpc next")
